...
<!--
Lives in `hangar/blueprints/skills/`. Defines the `loop-status` skill — a repeatable, on-demand procedure, invoked by name, with no persistent context of its own between invocations. Read by `construct`, which compiles it into whatever skill format the detected tool expects. Borrows its structural shape from loop engineering (see `docs/reading-list.md`) and from `the-architect.md`, rather than inventing a separate one.
-->

## Role

Produces one thing: a short block showing where the current work actually
stands — trigger, current agent, skills used so far, step count (exact
for a fixed cascade, honest and qualitative for open-ended work),
plain-language context, and next step — grounded directly in loop
engineering's own structural anatomy (trigger, topology, verifier, stop
rule; see `docs/reading-list.md` and `the-architect.md`, which this
skill borrows its shape from rather than inventing a separate one) so
that whoever's reading — the person, or a future session picking this
back up — can tell what's happening and where without having to
reconstruct it from scrollback.

**Use this skill**: whenever the person asks "where are we," "status,"
"onde estamos," or any similar phrasing in any form; or proactively at
natural checkpoints (the start and end of a multi-step cascade like
Foundation Sync, or a spec-driven implementation with several pieces) —
...
- **On demand, always** — the person asking "where are we," "status,"
  "onde estamos," or invoking this skill by name, in any phrasing, gets
  this block.
- **Proactively, at real checkpoints only** — the start of a multi-step
  ...
  the kind of context bloat this harness's own `deneir` was just built
